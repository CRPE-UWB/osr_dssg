...
##Enrollment##
```{r}
enrollment_copy <- s3read_using(FUN = read.csv, object = "s3://dssg2018/rawdata/DPS_Enrollment_1112-1718.csv")

head(enrollment_copy)
summary(enrollment_copy)
```
...
# School number is in the school information dataframe, so we can remove it
enrollment$SchoolNum <- NULL
...
colnames(attendance) <- make.names(gsub("\\d{4}.","",colnames(attendance)))
...
Finally, we visualize the attendance data.

```{r}
for (i in 2:ncol(attendance))
  hist(attendance[,i],main=paste(colnames(attendance)[i],"attendance"))
```
...
cmas[c(ela_score_inds,math_score_inds),-1] <- as.numeric(cmas[c(ela_score_inds,math_score_inds),-1])
cmas[c(ela_proficiency_inds,math_proficiency_inds,-1)] <- as.character(cmas[c(ela_proficiency_inds,math_proficiency_inds,-1)])

ela_scores <- cmas[-(1:2),c(1,ela_score_inds)]
colnames(ela_scores) <- c("2014-2015","2015-2016","2016-2017")
...
colnames(ela_proficiency) <- c("2014-2015","2015-2016","2016-2017")
...
colnames(math_scores) <- c("2014-2015","2015-2016","2016-2017")
...
colnames(math_proficiency) <- c("2014-2015","2015-2016","2016-2017")
math_proficiency <- math_proficiency %>% gather("2014-2015","2015-2016","2016-2017",key="SpringYear",value="MathProficiency")

# cmas <- bind_rows(ela_scores, ela_proficiency, math_scores, math_proficiency)
# glimpse(discipline)
```
