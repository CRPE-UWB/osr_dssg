...
Note that the so long as we call the access index from the new data, we will automaticaly subset our dataframe of travel times to the addresses in that dataframe, per the specifications of the user, so addresses which have been removed should not be double counted. 
...
###Read New Data
Assuming it has been preprocessed to match the blue print for summer data used in this project, see the preprocessing markdown for more. **NOTE** we don't actually have any new data, so here we make some up. Specifically, we session at a new address, and delete five elsewhere. We end up with one new unique address, but five fewer sessions at pre-existing addresses. .
```{r}
...
Merge_Set_New <- function(dataframe){
 #here we're cutting duplicate program_addresses, and other antiquated info (previous aggregations, PCT vhcl ownership) that was use in previous calculations
  temp <- merge(block_distance,dataframe,by="session_address_1")
...
```{r}
temp <- merge(new_reschool_programs,block_distance,by="session_address_1")
nrow(temp)
length(unique(temp$session_address_1))
length(unique(temp$Id2))

length(unique(new_reschool_programs$session_address_1))
nrow(new_reschool_programs)
nrow(temp)/481
```
...
Here we do a quick identical check to ensure the new driving & transit indecies are correct. In both cases identical should == false, even though access overall shouldn't budge much with a net loss of 4 sessions. 
```{r}
#verifying driving is different
summary(driving_index$AI_overall)
summary(new_driving_index$AI_overall)
identical(driving_index,new_driving_index) #if this returns FALSE, we've succesfully changed the addresses used in the calculation 

#verifying transit is different
summary(transit_index$AI_overall)
summary(new_transit_index$AI_overall)
identical(transit_index,new_transit_index) #again, we want this to return false
```
