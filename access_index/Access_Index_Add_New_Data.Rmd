...
google_api_key <- "AIzaSyBZa-H2jFWxs2TaDZg-PhipavCVA_FMp0I"
```
...
reschool_programs <- read.csv("../data/shiny_tables/old_b4s_programs.csv",
                              stringsAsFactors=FALSE)
...
If you want to run this just as a test, you have to uncomment the bottom line ( "write.csv(...)" )
```{r eval=FALSE}
...
new_reschool_programs <- new_reschool_programs[-c(1,20,40),]
...
#write.csv(new_reschool_programs, "../../data/shiny_tables/b4s_programs.csv")
```

###Find New Programs
This is a bit of a process. If there are new addresses, it will take about 10 minutes
```{r}
# First read in the new programs
new_reschool_programs <- read.csv("../data/shiny_tables/b4s_programs.csv")
...
if (length(address_vector)==0){
  ...
  block_distance <- merge(new_block_distance,block_distance,by=c("Id2","session_address_1","driving_morning","walking_morning","transit_morning","lat","long","kilometers"),all=TRUE)
  ############################################################################
...
Make_Subset <- function(dataframe,type=NULL,cost=NULL){
  ...
  if(is.null(cost)){
    return(dataframe)
     ...
     dataframe <- dataframe[dataframe[,cost],]
  }
  return(dataframe)
}
