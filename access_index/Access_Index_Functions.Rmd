...
The only interesting one here is low cost. Selam specifically recommended 50 dollars, though this was very low n for total session cost. HOWEVER- now that we have cost per day calculations, we can confirm that for non-free programs, the first quantile cost per is 47.5 dollars, and the third quantile is 54.75 dollars, where sessions costing $50 or less per day account for about 41.7% of all sessions. 
```{r}
...
reschool_programs$lowcost <- reschool_programs$cost_per_day<=50
...
driving_index <- summary_nature_driving
driving_index$AI_overall <-(summary_nature_driving$AccessIndex+summary_sports_driving$AccessIndex+summary_art_driving$AccessIndex+summary_academic_driving$AccessIndex)/4 #note that we can't use sum because that delivers col sums per each dataframe
...
census_blocks <- census_blocks[order(census_blocks$Id2),]
merged_set <- merged_set[order(merged_set$Id2),]
driving_index$Id2 <- as.numeric(driving_index$Id2)
...
range <- range(driving_index$AI_overall)
...
Ai_driving_normal <- leaflet(census_blocks) %>%
  addTiles() %>%
  addPolygons(color = "RdYlBu",weight =1,smoothFactor = 0.5,opacity = 0.5,fillOpacity = 0.75,
              fillColor = ~pal(driving_index$AI_overall),
              popup=paste("AI =",round(driving_index$AI_overall,digits=2)),
              highlightOptions = highlightOptions(color = "white", weight = 0.1)) %>%
  ...
  addCircles(data = agg_set, 
             ...
             color = ~pal2(agg_set$n) ) %>%
  setView(lat=39.7,lng=-104.9,zoom=11.5) %>%
  addLegend(pal = pal, values = ~driving_index$AI_overall, opacity = 1, title = "Access Index")

#Ordering
census_blocks$Id2 <- as.numeric(census_blocks$Id2)
merged_set$Id2 <- as.numeric(merged_set$Id2)
transit_index$Id2 <- as.numeric(transit_index$Id2)
...
range <- range(transit_index$AI_overall)
...
Ai_transit_normal <- leaflet(census_blocks) %>%
  addTiles() %>%
  addPolygons(color = "RdYlBu",weight =1,smoothFactor = 0.5,opacity = 0.5,fillOpacity = 0.9,
              fillColor = ~pal(transit_index$AI_overall),
              popup=paste("AI =",round(transit_index$AI_overall,digits=2)),
              highlightOptions = highlightOptions(color = "white", weight = 0.1)) %>%
  addLegend(pal = pal2, values = ~agg_set$n, opacity = 1,title = "Sessions") %>%
  addCircles(data = agg_set, 
             opacity = 1,weight=7, popup=info,
             color = ~pal2(agg_set$n) ) %>%
  addLegend(pal = pal, values = ~transit_index$AI_overall, opacity = 1, title = "Access Index Transit") %>%
  setView(lat=39.7,lng=-104.9,zoom=11.5)
```
