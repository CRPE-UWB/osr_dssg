...
Merge_Set <- function(dataframe){
  block_distance_new <- block_distance[,-c(3,4,9:14)] #here we're cutting duplicate program_addresses, and other antiquated info (previous aggregations, PCT vhcl ownership) that was use in previous calculations
  temp <- merge(block_distance_new,dataframe,all.y=TRUE)
  temp$n[is.na(temp$n)==TRUE] <- 0
  return(temp)
}

#5 Here we define the grabity threshold decay function. Note that as of July 25th we've decided not use this equation, and instead use the exponential equation per the results of the sensitivity analysis. 
