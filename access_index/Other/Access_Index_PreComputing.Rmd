...
###Driving Disabillity Dataframe
```{r}
#Create driving frame & subset for disability
driving_index_disabillity <- base_frame
reschool_programs_disabillity <- subset(reschool_programs,reschool_programs$has_special_needs_offerings==TRUE)
...
for (i in 1:length(categories)){
  ...
  for (x in 1:length(costs)){
    cost <- costs[x]
    user_set <- Make_Subset(reschool_programs_disabillity,type,cost)
    agg_set <- Aggregate_Subset(user_set)
    merged_set <- Merge_Set(agg_set)
    merged_set$AccessIndex <- decay_fxn_softGravity(merged_set,"drive") 
    summary_index_user <- aggregate(AccessIndex ~ Id2,data=merged_set,FUN=sum) 
  ...
  print(paste(i,"working"))
  if(length(driving_index_disabillity)>11){
    print(colnames(driving_index_disabillity))
  }
}
...
for (i in 1:length(categories)){
  ...
  print(paste(i,"working"))
  if(length(transit_index_disabillity)>11){
    print(colnames(transit_index_disabillity))
  }
}
...
data <- transit_index_disabillity  # must be already loaded in your environment
schemaName <- "clean"
tableName <- "transit_index_disabillity"
...
data <- driving_index_disabillity  # must be already loaded in your environment
schemaName <- "clean"
tableName <- "driving_index_disabillity"
