...
reschool_programs$last_start_time <- test$first_session_end_time
reschool_programs$Posfirst_start_time <- strptime(reschool_programs$first_start_time, "%I:%M:%S %p")
reschool_programs$Poslast_start_time <- strptime(reschool_programs$last_start_time, "%I:%M:%S %p")
reschool_programs$Hours_Day <- (reschool_programs$Poslast_start_time - reschool_programs$Posfirst_start_time)/(60*60) #differences are stored numerically in seconds, counting up from a static date. Thus we do last minus first for the difference in seconds, divide by 60*60 for to convert to hours
```
...
qplot(reschool_programs$cost_per_day,bins=200)
qplot(reschool_programs$session_cost,bins=200)
```
