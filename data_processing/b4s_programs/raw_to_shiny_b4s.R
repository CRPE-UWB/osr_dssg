...
# INPUT: dataframe of raw B4S programs data (csv) with at least the following columns (more is okay):
          ...
          # "session_date_start", "session_date_end", "first_session_start_time", "first_session_end_time"
...
raw_to_shiny_b4s <- function(raw_df, bg_nbhds_df) {
  ...
  char_cols <- c("camp_name", "session_name", "session_short_description", "session_date_start", 
                 "session_date_end", "session_address_name", "session_address_1", "session_address_2",
  ...
  # delete programs with bad zipcodes, take unique addresses
  bad_rows <- which(is.na(camp_address$session_zip))
  camp_address <- camp_address[-bad_rows,]
  ...
  dataPath <- file.path(local_path, "SHINY", "data_2018", "census_clean", "shape_census")
  ...
  bg_nbhds <- read.csv( file.path(local_path, "SHINY", "data2018", "bg_nbhds.csv") )
  ...
  final_cols_to_keep <- c("session_name", "first_session_date", "last_session_date", "session_cost", "lat", "long",
                          ...
                          "session_address_1", "session_zip", "session_city", "session_id", "cost_per_day"
                          )
