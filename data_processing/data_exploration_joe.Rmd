...
merged <- merge(block_locations,blocks_with_student_pop,all.x=TRUE)
...
merged$color <- 1
merged$color[merged$student_pop>=3] <- 2
merged$color[merged$student_pop>=6] <- 3
merged$color[merged$student_pop>=9] <- 4
merged$color[merged$student_pop>=12] <- 5

lab_factor <- c("Less than 3", 
                "Between 3 and 6", 
                "Between 6 and 9", 
                "Between 9 and 12",
                "Greater than 12")

merged$color <- factor(merged$color, label = lab_factor)
...
leaflet() %>% 
  ...
  addCircleMarkers(
    data = merged,
    stroke = FALSE, fillOpacity = .1, popup = paste("Student population in block:",merged$student_pop),
    color = ~colorFactor(colors5,merged$color)(merged$color)
  ) %>%
...
```{r}
merged_block_groups <- aggregate(student_pop ~ block_group, data=merged[,c("block_group","student_pop")], sum)
...
merged_block_groups@data$color <- 1
merged_block_groups@data$color[merged_block_groups@data$student_pop>=50] <- 2
merged_block_groups@data$color[merged_block_groups@data$student_pop>=100] <- 3
merged_block_groups@data$color[merged_block_groups@data$student_pop>=150] <- 4
merged_block_groups@data$color[merged_block_groups@data$student_pop>=200] <- 5

lab_factor <- c("Less than 50", 
                "Between 50 and 100", 
                "Between 100 and 150", 
                "Between 150 and 200",
                "Greater 200")

merged_block_groups@data$color <- factor(merged_block_groups@data$color, label = lab_factor)
# end set up for visualization
##############
...
leaflet() %>% 
  ...
  addPolygons(
    ...
    fillColor = ~colorFactor(colors5,merged_block_groups@data$color)(merged_block_groups@data$color),
    fillOpacity=10,
    popup = paste("Student population:",merged_block_groups@data$student_pop)
    ) 
...
block_and_census_pop <- merge(block_and_census_pop,census[,c("id2",age_subset)],all.x=TRUE,by.x="Id2",by.y="id2")
block_and_census_pop@data$census_pop <- apply(X = block_and_census_pop@data[,age_subset], FUN = sum, MARGIN=1)

differences = block_and_census_pop@data$frequencies-block_and_census_pop@data$census_pop
percent_differences <- round(differences / block_and_census_pop@data$frequencies,1)
hist(percent_differences,xlab="(Student - Census)/Student")
leaflet() %>% 
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(
    data = block_and_census_pop, weight=1,
