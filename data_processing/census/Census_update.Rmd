...
## Update census demographics


```{r}
setwd("~/Desktop/OSR2019/DATA/census_demo")
```

### Census data on block group level, Denver County. From ACS 2016. Estimates are based on 5 year average (2012-2016)
...
```{r}
# get data 
population <- read.csv("ACS_17_5YR_B00001.csv", skip = 1)
colnames(population)[which(names(population) == "Estimate..Total")] <- "TTL_population" #rename columns
```

#### Sex
```{r}
#the file from AFF is age by sex
sexage <- read.csv("ACS_17_5YR_B01001.csv", skip = 1)
#first look at sex
sex <- sexage %>% 
  select(c("Id","Estimate..Male.", "Estimate..Female."))
colnames(sex)[2:3] <- c("Male", "Female")
...
```{r}
race_ME <- read.csv("ACS_17_5YR_B02001_with_ann.csv", skip = 1)
#race
race <- race_ME%>% 
  select(-contains("Margin")) 
#Extracting the names of the columns that need to be modified
...
hispanic <- read.csv("ACS_17_5YR_B03003.csv", skip = 1)
...
``` {r}
census_edu_ME <- read.csv("ACS_17_5YR_B15003.csv", skip = 1)
...
```{r}
median_hh_income <- read.csv("ACS_17_5YR_B19013.csv", skip = 1)
...
#### Nativity
```{r}
nativity_ME <- read_csv("ACS_17_5YR_B99051.csv", skip = 1)
...
#### Housing tenure
```{r}
housing <- read.csv("ACS_17_5YR_B25044.csv", skip = 1)
...
#### Language
``` {r}
languagedata = read.csv(file = "ACS_17_5YR_B16004.csv", skip = 1, na.strings = "")
...
language$TTPOP_18PLUS_LNG <- rowSums(language[, c(5,11)])
language$LNG_English <- rowSums(language[,c(6,12)])
language$LNG_Spanish <- rowSums(language[,c(7,13)])
language$LNG_Spanish_English_bad <- rowSums(language[,c(8,14)])
language$LNG_IndoEuropean <- rowSums(language[,c(9,15)])
language$LNG_IndoEuropean_English_bad <- rowSums(language[,c(10,16)])
language$LNG_AsianPacific <-rowSums(language[,c(11,17)])
language$LNG_AsianPacific_English_bad <- rowSums(language[,c(12,18)])
language$LNG_Others <- rowSums(language[,c(13,19)])
language$LNG_Others_English_bad <- rowSums(language[,c(14,20)])
...
```{r}
#read in data
homecomposition <- read.csv("ACS_17_5YR_B17010.csv", skip = 1)
...
head(census)
write.csv(census, file = "Denver_Demographics_BG_2017.csv")
```

### Update shapefile

```{r}
shape_conorado <- readOGR("tl_2017_08_bg", "tl_2017_08_bg")

# read in the updated census csv file
census <- read.csv("Denver_Demographics_BG_2017.csv")
