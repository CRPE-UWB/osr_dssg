...
I'll sssume you're opening this in RStudio, so we set the working directory as follows. (Note that you'll need to change this if you're not using RStudio - the point is to set the working directory to where this file lives, so that data files are stored there too instead of somewhere random on your computer.)

```{r}
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))  # works for RStudio only
```
...
SavePolygonsAsCSV(skateParks, "skateParks.csv")
```
...
colnames(courtsSmall@data) <- c('type', 'location', 'year_built', 'year_resurfaced')
...
```{r}
filenames <- c("afterschool.csv", "courts.csv", "fields.csv") # add the rest, too
