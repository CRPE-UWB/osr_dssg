...
# look at AI_overall for driving first 
summary(data$AI_overall.x)

Quantile1_d <- quantile(data$AI_overall.x)[2]
Median_d <- median(data$AI_overall.x)
Quantile3_d <- quantile(data$AI_overall.x)[4]

data <- data %>%
  mutate( pct_lhs = less_than_hs_diploma *100/ttpop_25plus_edu,
          pct_hs = hsgread_or_equiv_edu *100/ttpop_25plus_edu,
          pct_sc = somecollege_or_aa_edu*100/ttpop_25plus_edu,
          pct_ab = bachelors_or_higher_edu*100/ttpop_25plus_edu,
          pct_native = native/ttl_population,
          mnb1stQ = ifelse(AI_overall.x < Quantile1_d,1,0), #Below_1st_Quantile
          mnbmedian = ifelse(AI_overall.x < Median_d,1,0), #Below_Median
          mnamedian = ifelse(AI_overall.x > Median_d,1,0), #Above_Median
          mna4stQ =  ifelse(AI_overall.x > Quantile3_d,1,0)) #Above_3rd_Quantile

col = c('median_hh_income','pct_hispanic','pct_white','pct_white','poverty_pct_hh','pct_lhs', 'pct_hs', 'pct_sc',
        'pct_ab', 'pct_native', 'pct_vehicle_own')

# create the data for visualization

# define the function to create the samples
create_samples <- function(df) {
...
sample_comparison = 
  bind_rows(
    ...
    filter(mnb1stQ == 1) %>%
    select(-mnb1stQ) %>%
    mutate(sample = 'Bottom 25 Percent'),
    ...
    filter(mnbmedian == 1) %>%
    select(-mnbmedian) %>%
    mutate(sample = 'Bottom 50 Percent'),
    ...
    filter(mna4stQ == 1) %>%
    select(-mna4stQ) %>%
    mutate(sample = 'Top 25 Percent')
)

    return(sample_comparison)
}

sample_comparison <- create_samples(data)

sample_comparison$sample <- 
  factor(sample_comparison$sample, 
         levels = c("Bottom 25 Percent", "Bottom 50 Percent", "Top 50 Percent", "Top 25 Percent"))
```

Graphics
``` {r}
# graph for median household income 
plot_MHI <- ggplot(sample_comparison, aes(sample, median_hh_income_mean)) +
  geom_bar(stat = "identity", fill = "#2166ac")  +
  xlab("Access Index Quantiles")+
  ylab("Median Household Income") +
  ggtitle("Median Household Income for Areas with Different Access Levels", 
          subtitle = "Driving Index for Overall Programs") +
  theme_tufte()

plot_MHI
```

REPLACED this one with racial compostion using DPS data
``` {r}
...
sample_compare_ses <- sample_comparison %>% 
  select(sample, pct_lhs_mean, pct_ab_mean, poverty_pct_hh_mean) %>%
  gather(ses, value, -sample) 
sample_compare_ses$ses <-
  factor(sample_compare_ses$ses,
  levels = c("pct_lhs_mean", "pct_ab_mean", "poverty_pct_hh_mean"))
plot_edu_poverty <- 
...
# poverty does not look super insightful, therefore why not make this just a pure comparison for education attainment? 

sample_compare_edu <- sample_comparison %>% 
  select(sample, pct_lhs_mean, pct_hs_mean, pct_ab_mean, pct_sc_mean) %>%
  gather(edu, value, -sample) 
sample_compare_edu$edu <-
  factor(sample_compare_edu$edu,
  levels = c("pct_lhs_mean", "pct_hs_mean", "pct_sc_mean", "pct_ab_mean"))

plot_edu <- 
  ggplot(sample_compare_edu) +
  geom_bar(aes(sample, value, fill = edu), stat = "identity", position = "dodge") +
  xlab("Access Index Quantiles") +
  ylab("Percentages") +
  ggtitle("Education Attainments for Areas with Different Levels of Access to OSR") +
...
plot_native <- ggplot(sample_comparison, aes(sample, pct_native_mean)) +
  geom_bar(stat = "identity", position = "dodge", fill = "#2166ac") +
  xlab("") +
  ylab("Percentages") +
  ggtitle("Nativity for Areas with Different Levels of Access to OSR") +
  theme_tufte()
...
# merge and tidy data before it goes into the function first. 

colnames(student_DPS_bg)[1] <- "Id2"
student_DPS_bg <- merge(student_DPS_bg, selected_index, by = "Id2") 
student_DPS_bg_drive <- student_DPS_bg %>%
  dplyr::mutate( mnb1stQ = ifelse(AI_overall.x < Quantile1_d,1,0), #Below_1st_Quantile
          mnbmedian = ifelse(AI_overall.x < Median_d,1,0), #Below_Median
          mnamedian = ifelse(AI_overall.x > Median_d,1,0), #Above_Median
          mna4stQ =  ifelse(AI_overall.x > Quantile3_d,1,0)) #Above_3rd_Quantile

col = colnames(student_DPS_bg_drive)[2:8]
student_comparison <- create_samples(student_DPS_bg_drive)
...
ggplot(plotdata, aes(sample, value, fill = Race)) +
  ...
  xlab("Access Index Quantiles") +
  ylab("Percentages") +
  ggtitle("Student Racial Compostion for Areas with Different Levels of Access to OSR") +
...
Race and break down of the indices
```{r}
selected_index2  <- full_index %>% select(Id2, AI_overall.x, AI_overall_free.x, AI_has_nature_anycost.x, AI_has_sports_anycost.x, AI_art_anycost.x, AI_academic_anycost.x, AI_overall.y)
...
result <- sapply(indices, get_race_access_means)
...
ggplot(student_comparison, aes(sample, perc_nonenglish_students_mean)) +
  geom_bar(stat = "identity", fill = "#2166ac") +
  xlab("Access Index Quantiles") +
  ylab("Percentages") +
...
```{r}
student_DPS_bg_raw <- dbGetQuery(con, "select * from public.dps_student_aggregate_bgs")
colnames(student_DPS_bg_raw)[1] <- "Id2"
student_AIscores <- merge(student_DPS_bg_raw, selected_index, by = "Id2")
```
