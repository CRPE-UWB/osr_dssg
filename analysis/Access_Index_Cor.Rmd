...
```{r}
cor_tests <- colnames(dps_agg)[2:22] #prepare dataframe
index_tests <- colnames(dps_agg)[24:length(colnames(dps_agg))] #prepare things we want to loop
...
###Loop for transit 
```{r}
dps_agg_transit <- dps_agg[-c(24:36)]
dps_agg_transit <- merge(dps_agg_transit,transit_index)

cor_tests <- colnames(dps_agg_transit)[2:22] #prepare dataframe
index_tests <- colnames(dps_agg_transit)[24:length(colnames(dps_agg_transit))] #prepare things we want to loop
cor_table <- data.frame(cor_tests)
