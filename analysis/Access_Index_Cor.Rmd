...
```{r}
cor_tests <- colnames(dps_agg)[2:21] #prepare dataframe
index_tests <- colnames(dps_agg)[23:length(colnames(dps_agg))] #prepare things we want to loop
...
cor_tests <- colnames(dps_agg_transit)[2:21] #prepare dataframe
index_tests <- colnames(dps_agg_transit)[23:length(colnames(dps_agg_transit))] #prepare things we want to loop
cor_table <- data.frame(cor_tests)
