...
decay_fxn_transit <- function(dataframe){
  scale=5
  dataframe$n*(ifelse(dataframe$transit_morning<=scale,1,(1/(dataframe$transit_morning/scale))^2))
}
...
#normalize
summary_index_user_transit$AccessIndex <- normalize(summary_index_user_transit$AccessIndex,max_access) #rescale(summary_index_user_transit$AccessIndex, to = c(0, 100))
```
...
```{r}
hist(block_distance$driving_morning)
hist(block_distance$transit_morning)
hist(summary_index_user$AccessIndex)
hist(summary_index_user_transit$AccessIndex)
```
