...
```{r}
library(ggmap)
register_google(key = google_api_key) #key stored outside github
```
...
for (i in 1:5){ 
  ...
  for (x in 1:nrow(block_mover)){
    ...
    mode="driving",
    arrival_time = as.POSIXct("2018-07-12 08:00:00 MT"),
    key = google_api_key)
    ...
    destination = c(lat.x,long.x),
    mode="driving",
    arrival_time = as.POSIXct("2018-07-12 12:00:00 MT"),
    key = google_api_key)
    distance_walking.x <- google_distance(origin=c(lat.i,long.i),
    destination = c(lat.x,long.x),
    mode="walking",
    ...
    mode="driving",
    arrival_time = as.POSIXct("2018-07-12 19:00:00 MT"),
    key = google_api_key)
    ...
    block_mover$driving_evening[x] <- as.numeric(distance.x$duration[2]/60) #paste drive time, etc, in minutes
...
###Final Loop
Given that differences in arrival times seem quite small, here we run the full loop for 08:00 departures. 
```{r}
library(ggmap)
library(googleway)
block_distance <- reschool_addresses
